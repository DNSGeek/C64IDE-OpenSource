...
final class SIDInstrument {
    ...
    /// Clamps an ADSR field to the 4 bits its register provides. The UI and
    /// sanitize() both keep these in range, but a register accessor must not
    /// be able to trap on a stray value.
    private static func nibble(_ value: Int) -> Int { max(0, min(15, value)) }

    /// Combined ADSR register byte: upper nibble = attack, lower nibble = decay.
    var adsrAD: UInt8 { UInt8((SIDInstrument.nibble(attack) << 4) | SIDInstrument.nibble(decay)) }
    /// Combined ADSR register byte: upper nibble = sustain, lower nibble = release.
    var adsrSR: UInt8 { UInt8((SIDInstrument.nibble(sustain) << 4) | SIDInstrument.nibble(release)) }
...
final class SIDPattern {
    /// Longest pattern the exports can describe: both `sid_pattern_lengths`
    /// and the BASIC row loop address a pattern's row count as a single byte.
...
final class SIDSong {
    ...
    /// Register writes for the global filter and volume registers
    /// ($D415-$D418). Nothing here changes while a song plays, so drivers and
    /// exports emit them once at start-up rather than on every row.
    func globalRegisterWrites() -> [(register: UInt8, value: UInt8)] {
        let cutoff    = max(0, min(2047, filterCutoff))
        let resonance = max(0, min(15, filterResonance))
        let volume    = max(0, min(15, globalVolume))
        return [
            (0x15, UInt8(cutoff & 0x07)),                           // Filter cutoff lo (3 bits)
            (0x16, UInt8(cutoff >> 3)),                             // Filter cutoff hi (8 bits)
            (0x17, UInt8(resonance << 4) | (filterVoices & 0x07)),  // Resonance + filter voice routing
            (0x18, (filterType & 0x70) | UInt8(volume)),            // Filter mode + volume
        ]
    }

    /// Generates SID voice register writes for a given pattern and row.
    /// Filter and volume are not included — see `globalRegisterWrites()`.
    ...
    /// Formats a register-write list as the `count, reg, val, ...` byte
    /// sequence both exports and the driver format use.
    private func writeBlock(_ writes: [(register: UInt8, value: UInt8)]) -> String {
        guard !writes.isEmpty else { return ".byte 0" }
        let data = writes.flatMap { [String(format: "$%02X", $0.register), String(format: "$%02X", $0.value)] }
        return ".byte \(writes.count), \(data.joined(separator: ", "))"
    }

    /// Exports the song as 6502 assembly data containing register writes.
    func exportAsAssembly() -> String {
        // Every count below is read back as a single byte by the driver.
        // sanitize() clamps pattern lengths and the sequence length so they
        // always fit; the prefix/min here keep a hand-built song honest too.
        let seqCount = min(sequence.count, 255)
        ...
        var lines = [
            ...
            "sid_pattern_count:   .byte \(min(patterns.count, 255))",
            "sid_sequence_length: .byte \(seqCount)",
            "sid_sequence:",
            "    .byte \(sequence.prefix(seqCount).map { String($0) }.joined(separator: ", "))",
            "",
            "; Rows in each pattern, indexed by pattern number. Row data is",
            "; variable length, so a player needs this to know where to stop.",
            "sid_pattern_lengths:",
            "    .byte \(patterns.map { String(min($0.length, 255)) }.joined(separator: ", "))",
            "",
            "; Filter and volume never change while the song plays, so they are",
            "; written once at start-up instead of on every row.",
            "; Format: num_writes, [reg, val, reg, val, ...]",
            "sid_init:",
            "    \(writeBlock(globalRegisterWrites()))",
            "",
        ]

        for (pi, pattern) in patterns.enumerated() {
            lines.append("sid_pattern_\(pi):  ; \(pattern.length) rows")
            lines.append("    ; Format: num_writes, [reg, val, reg, val, ...]")

            for row in 0..<pattern.length {
                let writes = registerWrites(pattern: pi, row: row)
                let suffix = writes.isEmpty ? "  ; row \(row) (empty)" : "  ; row \(row)"
                lines.append("    \(writeBlock(writes))\(suffix)")
            }
            lines.append("")
        }
    ...
    /// Highest line number a C64 BASIC program can use. A listing numbered
    /// past this cannot be entered or LISTed on real hardware.
    private static let maxBASICLineNumber = 63999

    /// Exports the song as BASIC POKE statements.
    /// A long song needs more line numbers than BASIC has; when that happens
    /// the output stops at the last usable line, still switches the volume
    /// off, and says so in a REM at the top rather than emitting a listing
    /// the C64 would reject.
    func exportAsBASIC() -> String {
        ...
        // Reserve the final line number for the closing volume-off POKE.
        let lastUsableLine = SIDSong.maxBASICLineNumber - 10

        /// Emits one row's POKEs, packed onto as few lines as the C64's
        /// 80-character logical line limit allows (a longer line cannot be
        /// typed or re-entered on real hardware). Returns false when the
        /// program has run out of line numbers.
        func emit(_ writes: [(register: UInt8, value: UInt8)]) -> Bool {
            let maxLineLength = 78
            var pokes = writes.map { "POKE \(SID_BASE + UInt16($0.register)),\($0.value)" }
            while !pokes.isEmpty {
                guard lineNum <= lastUsableLine else { return false }
        ...
        // Filter and volume are constant for the whole song — set them once.
        if !emit(globalRegisterWrites()) { truncated = true }

        if !truncated {
            rows: for patIdx in sequence {
                guard patterns.indices.contains(patIdx) else { continue }
                ...
                for row in 0..<pattern.length {
                    ...
                    guard emit(writes) else { truncated = true; break rows }

                    // Delay
                    guard lineNum <= lastUsableLine else { truncated = true; break rows }
        ...
        if truncated {
            lines.insert("3 REM WARNING: SONG TRUNCATED - TOO LONG FOR BASIC", at: 2)
        }
...
extension SIDPattern: Codable {
    ...
    convenience init(from decoder: Decoder) throws {
        ...
        self.init(length: max(1, min(SIDPattern.maxLength, decodedLength)))
...
extension SIDSong: Codable {
    ...
    func sanitize() {
        ...
        for pattern in patterns {
            pattern.length = max(1, min(SIDPattern.maxLength, pattern.length))
        ...
        // The exported sequence length is a single byte.
        if sequence.count > 255 { sequence.removeLast(sequence.count - 255) }
