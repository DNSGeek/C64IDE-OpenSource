...
public struct C64Reference {
    ...
    public static let zeroPageEntries: [MemoryMapEntry] = [
        ...
        MemoryMapEntry(address: 0x0090, endAddress: nil, name: "ST (Status variable)", chip: .ram,
            description: "Device status for serial bus/datasette. Updated after every I/O operation. Meanings are device-dependent (serial bus shown here).",
            bits: [
                BitFieldEntry(bits: "0", name: "Write timeout", description: "1=Timeout on serial bus output (write-side)"),
                BitFieldEntry(bits: "1", name: "Read timeout", description: "1=Timeout on serial bus input (read-side)"),
                BitFieldEntry(bits: "4", name: "Verify error", description: "1=Verify error (VERIFY only) / checksum error (tape)"),
                BitFieldEntry(bits: "6", name: "EOF", description: "1=End of file (serial) / end of record (tape)"),
                BitFieldEntry(bits: "7", name: "Not present", description: "1=Device not present or unrecoverable read error"),
            ]),
        ...
        MemoryMapEntry(address: 0x00A0, endAddress: 0x00A2, name: "TI (jiffy clock)", chip: .ram,
            description: "Time of day, incremented every jiffy by the KERNAL IRQ. 3 bytes: $00A0=high, $00A1=mid, $00A2=low. Wraps at 5184000 jiffies (= 24 hours). The C64 KERNAL normalizes CIA1 Timer A to fire at ~60 Hz on both PAL and NTSC machines, so the 24-hour wrap applies equally to both.", bits: nil),
    ...
    public static let stackAndBufferEntries: [MemoryMapEntry] = [
        ...
        MemoryMapEntry(address: 0x0311, endAddress: 0x0312, name: "USR() vector", chip: .ram,
            description: "16-bit pointer (lo/hi) to the machine language routine called by the BASIC USR() function. Set low byte at 785, high byte at 786 before calling USR(x). The routine receives the argument in the floating-point accumulator (FAC1) and should return a result in FAC1.",
            bits: nil),
    ...
    public static let cia2Registers: [MemoryMapEntry] = [
        ...
        MemoryMapEntry(address: 0xDD02, endAddress: nil, name: "CIA2 Port A DDR", chip: .cia2,
            description: "Data direction for Port A. Default: $3F (bits 0-5 output: VIC bank, RS232 TXD, serial ATN/CLK/DATA out; bits 6-7 input: serial CLK/DATA in).", bits: nil),
        MemoryMapEntry(address: 0xDD03, endAddress: nil, name: "CIA2 Port B DDR", chip: .cia2,
            description: "Data direction for Port B (user port / RS-232). Bit=0: input, Bit=1: output. Default: $00 (all input).", bits: nil),
    ...
    public static let kernalAreaEntries: [MemoryMapEntry] = [
        MemoryMapEntry(address: 0xE000, endAddress: 0xFFFF, name: "KERNAL ROM / RAM", chip: .kernal,
            description: "8KB KERNAL ROM ($E000-$FFFF). Banked via $0001 bit 1: 1=KERNAL ROM visible, 0=RAM. Contains screen editor, I/O routines, and the KERNAL jump table at $FF81-$FFF3.", bits: nil),
    ...
    public static let kernalRoutines: [KernalRoutine] = [
        ...
        KernalRoutine(address: 0xFF8A, name: "RESTOR", description: "Fill vector table at $0314-$0333 with default values.", input: nil, output: nil, usedRegisters: "None", realAddress: 0xFD15),
        ...
        KernalRoutine(address: 0xFF90, name: "SETMSG", description: "Set system error display switch at $009D.", input: "A=Switch value", output: nil, usedRegisters: "None", realAddress: 0xFE18),
        ...
        KernalRoutine(address: 0xFF99, name: "MEMTOP", description: "Read or set the top of BASIC/system memory ($0283-$0284). Lower it to reserve space for machine code above BASIC.", input: "Carry: 1=Read into X/Y, 0=Set from X/Y", output: "X=Low byte, Y=High byte (if Carry=1)", usedRegisters: "X, Y", realAddress: 0xFE25),
        KernalRoutine(address: 0xFF9C, name: "MEMBOT", description: "Read or set the bottom of BASIC/system memory ($0281-$0282). Defaults to $0800; BASIC only re-reads it at cold start.", input: "Carry: 1=Read into X/Y, 0=Set from X/Y", output: "X=Low byte, Y=High byte (if Carry=1)", usedRegisters: "X, Y", realAddress: 0xFE34),
        ...
        KernalRoutine(address: 0xFFA2, name: "SETTMO", description: "Set serial bus timeout.", input: "A=Timeout value", output: nil, usedRegisters: "None", realAddress: 0xFE21),
        ...
        KernalRoutine(address: 0xFFA8, name: "IECOUT", description: "Write byte to serial bus. Must call LISTEN and LSTNSA first.", input: "A=Byte to write", output: nil, usedRegisters: "None", realAddress: 0xEDDD),
        ...
        KernalRoutine(address: 0xFFBA, name: "SETLFS", description: "Set file parameters for OPEN.", input: "A=Logical number, X=Device number, Y=Secondary address", output: nil, usedRegisters: "None", realAddress: 0xFE00),
        KernalRoutine(address: 0xFFBD, name: "SETNAM", description: "Set file name parameters.", input: "A=Name length, X/Y=Pointer to name (lo/hi)", output: nil, usedRegisters: "None", realAddress: 0xFDF9),
        ...
        KernalRoutine(address: 0xFFD2, name: "CHROUT", description: "Write byte to default output. Must call OPEN+CHKOUT first for non-screen.", input: "A=Byte to write", output: nil, usedRegisters: "None", realAddress: 0xF1CA),
        ...
        KernalRoutine(address: 0xFFDB, name: "SETTIM", description: "Set the software jiffy clock at $00A0-$00A2 (BASIC's TI/TI$). Not the CIA Time of Day clock.", input: "A/X/Y=New TOD value", output: nil, usedRegisters: "None", realAddress: 0xF6E4),
        KernalRoutine(address: 0xFFDE, name: "RDTIM", description: "Read the software jiffy clock at $00A0-$00A2 (BASIC's TI/TI$). Counts 1/60s ticks on NTSC, 1/50s on PAL, and wraps after 24 hours.", input: nil, output: "A/X/Y=Current TOD value", usedRegisters: "A, X, Y", realAddress: 0xF6DD),
        ...
        KernalRoutine(address: 0xFFEA, name: "UDTIM", description: "Advance the jiffy clock ($00A0-$00A2) by one tick and update the Stop key indicator ($0091). The 60Hz IRQ calls this; a custom IRQ that bypasses $EA31 must call it or TI and STOP both freeze.", input: nil, output: nil, usedRegisters: "A, X", realAddress: 0xF69B),
